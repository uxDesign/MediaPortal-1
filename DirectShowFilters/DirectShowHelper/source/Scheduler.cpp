...
UINT CALLBACK DispTimThread(void* param)
{  
  ...
  int iFramesProcessedLast = 0;
  double dDtoVRatio = 1.0;
  ...
  if (m_pAvSetMmThreadPriority) 
  {
    ...
    {
      Log("DispTim AvSetMmThreadPriority = %d", (p->pPresenter->m_regTimerMmcssPriority - 1));
    }
  }
  ...
  LONGLONG lastDispCheckTime = GetCurrentTimestamp();

  while (!p->bDone)
  {       
    dwObject = WaitForMultipleObjects (2, hEvts, FALSE, 1);
    switch (dwObject)
    {
      case WAIT_OBJECT_0 :     //eHasWork
        p->eHasWork.Reset();
        continue; // check loop status
        break;
      case WAIT_OBJECT_0 + 1 : //eHasWorkLP
        p->eHasWorkLP.Reset();
        p->pPresenter->UpdatePhaseDiff();
        break;
      case WAIT_TIMEOUT :
        break;
    }

    currScanLine = p->pPresenter->GetScanLine();
    if ((currScanLine >= 0) && (currScanLine < lastScanLine))
    {
      p->pPresenter->m_iDisplayFrameCount++;
      
      if ((p->pPresenter->m_iDisplayFrameCount % 256) == 0)
      {
        LONGLONG timeNow = GetCurrentTimestamp();
        p->pPresenter->m_dDisplayPeriodRolling = (double)(timeNow - lastDispCheckTime)/(256 * 10000000.0);
        lastDispCheckTime = timeNow;
      }

      if ((p->pPresenter->m_iFramesProcessed > 50) && (p->pPresenter->m_dDispToVidRatio > 0.0))
      {
        iDisplayFrameCount++;
        
        double frameDiff = p->pPresenter->CompareFrameTimes();
        
        //Check for x1/x2/x0.5 video frame period (relative to nominal/orginal frame period)
        if (frameDiff > 0.985 && frameDiff < 1.015)
        {
          dDtoVRatio = p->pPresenter->m_dDispToVidRatio;
        }       
        else if (frameDiff > 1.97 && frameDiff < 2.03)
        {
          dDtoVRatio = p->pPresenter->m_dDispToVidRatio * 0.5;
        }       
        else if (frameDiff > 0.4925 && frameDiff < 0.5075)
        {
          dDtoVRatio = p->pPresenter->m_dDispToVidRatio * 2.0;
        }       
        else
        {
          dDtoVRatio = 1.0;
          iDisplayFrameCount = 0;
          iFramesProcessedLast = p->pPresenter->m_iFramesProcessed;
        }       

        p->pPresenter->m_dDToVdrift = ((double)iDisplayFrameCount - ((double)(p->pPresenter->m_iFramesProcessed - iFramesProcessedLast) * dDtoVRatio)) / dDtoVRatio;        
               
        if ((fabs(p->pPresenter->m_dDToVdrift)) > 1.5)
        {
          iDisplayFrameCount = 0;
          iFramesProcessedLast = p->pPresenter->m_iFramesProcessed;
          p->pPresenter->DriftAdjust((int)p->pPresenter->m_dDToVdrift);
        }       
      }
      else
      {
