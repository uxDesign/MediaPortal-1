  ...
  public class GUIMusicGenres : GUIMusicBaseWindow
  {
    ...
    protected override MusicSort.SortMethod CurrentSortMethod
    {
      ...
      set
      {
        ...
        FilterLevel def = (FilterLevel)handler.View.Levels[handler.CurrentLevel];
    ...
    protected override void OnPageLoad()
    {
      ...
      if (_loadParameter != null)
      {
        ...
        foreach (ViewDefinitionNew v in handler.Views)
        {
      ...
      {
        view = ((ViewDefinitionNew)handler.Views[0]).Name;
      }
    ...
    protected override void OnRetrieveCoverArt(GUIListItem item)
    {
      ...
      FilterLevel filter = (FilterLevel)handler.View.Levels[handler.CurrentLevel];
    ...
    private void InitLayoutAndSort()
    {
      ...
      foreach (ViewDefinitionNew view in handler.Views)
      {
        try
        {
        ...
        foreach (ViewDefinitionNew subview in view.SubViews)
        {
          ...
          int levelnr = 0;
          foreach (FilterLevel level in subview.Levels)
          {
            try
            {
