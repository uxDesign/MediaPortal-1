...
namespace TvEngine.PowerScheduler
{
  ...
  public class WindowsController
  {
    ...
    {
      Log.PSDebug("WindowsController: Setting ExitWindows to {0}.{1}", handler.Target, handler.Method);
    ...
    protected static void ExitWindowsDefaultThread(object _data)
    {
      ...
      Log.PSDebug("WindowsController: Performing ExitWindows {0}, force: {1}", how, force);
      bool res;
      switch (how)
      {
      ...
      Log.PSDebug("WindowsController: ExitWindows performed, result: {0}", res);
      if (env.after != null)
    ...
    {
      Log.PSInfo("--Exit Windows - ", how.ToString() + ", " + force.ToString());
